...
from astronet.astro_model_mario import configurations_mario
from astronet.ops import input_ops
from astronet.ops import testing
from astronet.util import configdict


class AstroModelTestMario(tf.test.TestCase):
  ...
  def testInvalidModeRaisesError(self):
    # Build config.
    config = configdict.ConfigDict(configurations_mario.base())

    # Build model.
    features = input_ops.build_feature_placeholders(config.inputs.features)
    labels = input_ops.build_labels_placeholder()
    with self.assertRaises(ValueError):
      _ = astro_model_mario.AstroModel(features, labels, config.hparams, "training")

  def testZeroFeaturesRaisesError(self):
    # Build config.
    config = configurations_mario.base()
    ...
    features = input_ops.build_feature_placeholders(config.inputs.features)
    labels = input_ops.build_labels_placeholder()
    model = astro_model_mario.AstroModel(features, labels, config.hparams,
  ...
  def testPredictMode(self):
    ...
    # Build model.
    features = input_ops.build_feature_placeholders(config.inputs.features)
    model = astro_model_mario.AstroModel(features, None, config.hparams,
