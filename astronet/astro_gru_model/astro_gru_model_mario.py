...
class AstroGRUModelMario(astro_model_mario.AstroModelMario):
    ...
    def _build_gru_layers(self, inputs, hparams, scope="bi-lstm"):
            ...
            for i in range(1):
                ...
                with tf.variable_scope("block_%d" % (i + 1)):
                    for j in range(1):
                        ...
                        gru = tf.keras.layers.GRU(num_hidden)
                        gru2 = tf.compat.v1.keras.layers.GRU(num_hidden)
                        gru_cell = tf.keras.layers.GRUCell(num_hidden)
                        ...
                        stacked_gru_output = tf.reshape(gru_output, [-1, 120])
                        stacked_outputs = tf.layers.dense(stacked_gru_output, 1)
