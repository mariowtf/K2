...
class AstroCNNModelMario(astro_model_mario.AstroModelMario):
    ...
    """
        super(AstroCNNModelMario, self).__init__(features, labels, hparams, mode)
        ...
        with tf.variable_scope(scope):
            ...
            for i in range(hparams.cnn_num_blocks):
                ...
                with tf.variable_scope("block_%d" % (i + 1)):
                    ...
                    if hparams.pre_logits_dropout_rate > 0:
                        net = tf.layers.dropout(
                            net,
                            hparams.pre_logits_dropout_rate,
                            training=True)

                    if hparams.pool_size > 1:
    ...
    def show_params(self):
        """
        Se puede utilizar tf.summary.text si se quiere ver en tensorboard
        Returns: imprime parámetros
        ...
        for v in tf.trainable_variables():
            ...
            total += num
            print('  %s \t\t Num: %d \t\t Tam %s ' % (v.name, num, dims))
        print('\nNumero de parámetros: %d' % total)